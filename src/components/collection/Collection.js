...
function CollectionItem(props) {
    return (
        <Link to={props.link} className={"group block relative overflow-hidden no-underline rounded-[10px] w-full"}>
            ...
            <Box sx={{backgroundImage: `url(${props.image})`}} className={"relative h-[400px] bg-center bg-cover bg-no-repeat"} />
            <Typography
                variant="subtitle1"
...
function Collection({data}) {
    return (
        <Box component={"section"} className={"section mb-8"}>
            <Box className={"container mx-auto"}>
                <Typography variant="h3" className={"text-center text-5xl font-bold uppercase mt-20 mb-8"}>
                    Bộ sưu tập
                </Typography>
                <Grid container spacing={2}>
                    {data?.map((item, index) => (
                        <Grid item xl={3} md={6} sm={6} xs={12} key={index}>
